...
namespace Controller
{
    ...
    public sealed class CellController : BaseControllerWithViewAndData<CellView, CellData>
    {
        ...
        {
            ApplyState(CellState.Hidden, 0, false);
        }

        public void ApplyState(CellState state, int adjacentMines, bool isMine)
        {
            View.SetSprite(ResolveSprite(state, adjacentMines, isMine));
        }

        private Sprite ResolveSprite(CellState state, int adjacentMines, bool isMine) => state switch
        {
